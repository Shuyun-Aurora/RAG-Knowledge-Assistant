...
const Register = () => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (result?.success) {
        messageApi.success(result.message || '注册成功，请登录');
        navigate('/login');
      } else {
  ...
  return (
    <div className="auth-bg" style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', minHeight: '100vh', position: 'relative' }}>
      {/* 左侧hello SVG艺术字动画 */}
      <svg
        width="500"
        height="180"
      ...
      <Card className="auth-card" style={{ marginRight: 120, padding: '16px 16px' }}>
        <div className="auth-title" style={{ marginBottom: 8 }}>
          ...
          <Text type="secondary">请选择身份并填写信息</Text>
        </div>
